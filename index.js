...
function inicializujAplikaciu() {
  const appObj = {
    ...
    aktualnyPouzivatelSezona: '',
    aktualnyPouzivatelKategoria: '',
    vsetciPouzivatelia: [],
...
// Upravená funkcia zobrazPouzivatelov - preferencie v samostatných stĺpcoch + červené podfarbenie pri AKOMKOĽVEK nesúlade
function zobrazPouzivatelov(pouzivatelia) {
  ...
  // Upravená funkcia na zistenie nezhody - AKÝKOĽVEK nesúlad
  function maNezhoduLocal(user) {
    ...
    const maAkejkolvekPriradenia = priradeneTimy.length > 0 || priradeneSezony.length > 0 || priradeneKategorie.length > 0;
    
    if (!maAkejkolvekPreferencie && !maAkejkolvekPriradenia) return false;
    
    // KĽÚČOVÁ ZMENA: Akýkoľvek nesúlad medzi preferenciami a priradeniami
    ...
    const maPridanuPreferenciu = 
      preferovaneTimy.some(tim => !priradeneTimy.includes(tim)) ||
      preferovaneSezony.some(sezona => !priradeneSezony.includes(sezona)) ||
      preferovaneKategorie.some(kategoria => !priradeneKategorie.includes(kategoria));
    
    return maOdstranenuPreferenciu || maPridanuPreferenciu;
  }
  ...
  const zoradeni = [...pouzivatelia].sort((a, b) => {
    ...
    const nezhodaA = maNezhoduLocal(a) && a.role !== 'admin';
    const nezhodaB = maNezhoduLocal(b) && b.role !== 'admin';
  ...
  zoradeni.forEach((user) => {
    ...
    // --- ZVÝRAZNENIE RIADKU - ČERVENÉ PODFARBENIE PRI AKOMKOĽVEK NESÚLADE ---
    let rowStyle = '';
    if (cakaNaSchvalenie) {
      rowStyle = 'background-color:#fff8e1;border-left:4px solid #ffc107;';
    } 
    // KĽÚČOVÁ ZMENA: Akýkoľvek nesúlad = červené podfarbenie
...
function getPreferenceChanges(user) {
  const changes = [];
  ...
  // Kontrola tímov - PRIDANÉ (sú v priradených ale nie v preferovaných)
  priradeneTimy.forEach(tim => {
    if (!preferovaneTimy.includes(tim)) {
      changes.push({ type: 'pridane', category: 'team', value: tim });
    }
  });
  
  // Kontrola tímov - ODSTRÁNENÉ (sú v preferovaných ale nie v priradených)
  preferovaneTimy.forEach(tim => {
    if (!priradeneTimy.includes(tim)) {
      changes.push({ type: 'odstranene', category: 'team', value: tim });
    }
  });
  
  // Kontrola sezón - PRIDANÉ
  priradeneSezony.forEach(sezona => {
    if (!preferovaneSezony.includes(sezona)) {
      changes.push({ type: 'pridane', category: 'season', value: sezona });
    }
  });
  
  // Kontrola sezón - ODSTRÁNENÉ
  preferovaneSezony.forEach(sezona => {
    if (!priradeneSezony.includes(sezona)) {
      changes.push({ type: 'odstranene', category: 'season', value: sezona });
    }
  });
  
  // Kontrola kategórií - PRIDANÉ
  priradeneKategorie.forEach(kategoria => {
    if (!preferovaneKategorie.includes(kategoria)) {
      changes.push({ type: 'pridane', category: 'category', value: kategoria });
    }
  });
  
  // Kontrola kategórií - ODSTRÁNENÉ
  preferovaneKategorie.forEach(kategoria => {
    if (!priradeneKategorie.includes(kategoria)) {
      changes.push({ type: 'odstranene', category: 'category', value: kategoria });
    }
  });
...
window.otvorModalPriradeniaTimov = async function(userId) {
  // Najprv nájdeme používateľa v lokálnych dátach
  let user = window.app.vsetciPouzivatelia.find(u => u.id === userId);
  if (!user) {
  ...
  closeBtn.onclick = () => {
    modal.remove();
    // Odpojíme listener pri zatvorení
    if (window._unsubscribeModalUser) {
      window._unsubscribeModalUser();
      window._unsubscribeModalUser = null;
    }
  };
  ...
  const title = document.createElement('h3');
  title.textContent = `Priradiť preferencie pre: ${user.email}`;
  title.style.marginBottom = '10px';
  modalBox.appendChild(title);
  
  const infoText = document.createElement('p');
  infoText.textContent = 'Vyberte tímy, sezóny a kategórie, ktoré chcete priradiť používateľovi. Videá budú filtrované podľa týchto priradených hodnôt. Pôvodné preferované hodnoty z registrácie zostávajú zachované.';
  infoText.style.fontSize = '14px';
  infoText.style.color = '#666';
  infoText.style.marginBottom = '20px';
  modalBox.appendChild(infoText);
  
  // --- KONTAJNER PRE OBSAH (budeme ho aktualizovať) ---
  ...
  function vytvorModalnyObsah(userData) {
    ...
    if (userData.teamName) {
      if (Array.isArray(userData.teamName)) {
        aktualneTimy = userData.teamName;
      } else if (typeof userData.teamName === 'string' && userData.teamName.includes(',')) {
        aktualneTimy = userData.teamName.split(',').map(t => t.trim()).filter(t => t);
      } else if (typeof userData.teamName === 'string' && userData.teamName) {
        aktualneTimy = [userData.teamName];
      }
    }
    ...
    let aktualneSezony = [];
    if (userData.assignedSezona) {
      if (Array.isArray(userData.assignedSezona)) {
        aktualneSezony = userData.assignedSezona;
      } else if (typeof userData.assignedSezona === 'string' && userData.assignedSezona.includes(',')) {
        aktualneSezony = userData.assignedSezona.split(',').map(t => t.trim()).filter(t => t);
      } else if (typeof userData.assignedSezona === 'string' && userData.assignedSezona) {
        aktualneSezony = [userData.assignedSezona];
      }
    }
    ...
    if (userData.assignedKategoria) {
      if (Array.isArray(userData.assignedKategoria)) {
        aktualneKategorie = userData.assignedKategoria;
      } else if (typeof userData.assignedKategoria === 'string' && userData.assignedKategoria.includes(',')) {
        aktualneKategorie = userData.assignedKategoria.split(',').map(t => t.trim()).filter(t => t);
      } else if (typeof userData.assignedKategoria === 'string' && userData.assignedKategoria) {
        aktualneKategorie = [userData.assignedKategoria];
      }
    }
    ...
    if (userData.teamPreference) {
      if (Array.isArray(userData.teamPreference)) {
        preferovaneTimy = userData.teamPreference;
      } else if (typeof userData.teamPreference === 'string' && userData.teamPreference.includes(',')) {
        preferovaneTimy = userData.teamPreference.split(',').map(t => t.trim()).filter(t => t);
    ...
    if (userData.sezonaPreference) {
      if (Array.isArray(userData.sezonaPreference)) {
        preferovaneSezony = userData.sezonaPreference;
      } else if (typeof userData.sezonaPreference === 'string' && userData.sezonaPreference.includes(',')) {
        preferovaneSezony = userData.sezonaPreference.split(',').map(t => t.trim()).filter(t => t);
    ...
    if (userData.kategoriaPreference) {
      if (Array.isArray(userData.kategoriaPreference)) {
        preferovaneKategorie = userData.kategoriaPreference;
      } else if (typeof userData.kategoriaPreference === 'string' && userData.kategoriaPreference.includes(',')) {
        preferovaneKategorie = userData.kategoriaPreference.split(',').map(t => t.trim()).filter(t => t);
    ...
    contentContainer.appendChild(sezonaContainer);
    ...
    if (!contentContainer.querySelector('.modal-buttons')) {
      const buttonsDiv = document.createElement('div');
      buttonsDiv.className = 'modal-buttons';
      ...
      saveBtn.onclick = async () => {
        ...
        const timCheckboxes = timyContainer.querySelectorAll('input[type="checkbox"]');
        const vybraneTimy = [];
        ...
        try {
          ...
          if (updatedUser) {
            ...
            updatedUser.assignedSezona = vybraneSezony;
          ...
          let sprava = `✅ Používateľovi <strong>${userData.email}</strong> boli aktualizované priradené preferencie.<br><br>`;
          ...
          sprava += `🏆 <strong>Priradené kategórie:</strong> ${vybraneKategorie.length > 0 ? vybraneKategorie.map(k => categoryMap[k] || k).join(', ') : 'Žiadne'}`;
  ...
  // --- SPUSTENIE REAL-TIME LISTENERA NA POUŽÍVATEĽA ---
  // Odpojíme predchádzajúci listener ak existuje
  if (window._unsubscribeModalUser) {
    window._unsubscribeModalUser();
    window._unsubscribeModalUser = null;
  }
  ...
  const userRef = doc(db, 'users', userId);
  window._unsubscribeModalUser = onSnapshot(userRef, (docSnapshot) => {
    if (docSnapshot.exists()) {
      const freshUserData = docSnapshot.data();
      // Aktualizujeme lokálne dáta
      const localUser = window.app.vsetciPouzivatelia.find(u => u.id === userId);
      if (localUser) {
        localUser.teamPreference = freshUserData.teamPreference || '';
        localUser.sezonaPreference = freshUserData.sezonaPreference || '';
        localUser.kategoriaPreference = freshUserData.kategoriaPreference || '';
        localUser.teamName = freshUserData.teamName || '';
        localUser.assignedSezona = freshUserData.assignedSezona || '';
        localUser.assignedKategoria = freshUserData.assignedKategoria || '';
      }
      ...
      const updatedUserData = {
        ...
        sezonaPreference: freshUserData.sezonaPreference || '',
        kategoriaPreference: freshUserData.kategoriaPreference || '',
        teamName: freshUserData.teamName || '',
        assignedSezona: freshUserData.assignedSezona || '',
    ...
    console.warn('Chyba pri real-time listeneri na používateľa:', error);
  });
  
  // --- PRIDANIE MODÁLU DO TELA ---
  modalBox.appendChild(contentContainer);
...
function zobrazVideaPouzivatelom(videa) {
  ...
  zobrazeneVidea = zobrazeneVidea.sort(sortByDate);
  ...
  if (!zobrazeneVidea || zobrazeneVidea.length === 0) {
    ...
    container.innerHTML = `
      ...
      <p style="text-align:center;color:#999;padding:40px;">${sprava}</p>
    `;
    return;
  }
...
window.aplikujFiltre = function() {
  ...
  // Najprv odfiltruj videá BEZ ID a aplikuj filtre
  let filtered = videa.filter(v => {
    // Najprv filter na ID - videá bez ID sa nezobrazia
    if (!v.videoId || v.videoId.trim() === '') return false;
  ...
  filtered = filtered.sort(sortByDate);
...
function nacitajVideaDoAdminPanelu() {
  ...
  // ROZDELENIE podľa ID pre admina
  ...
  // Zoraď obe skupiny a spoj - najprv BEZ ID (aby admin videl, čo treba opraviť), potom S ID
...
function zobrazVideaAdmin(videa) {
  ...
  // ROZDELENIE podľa ID pre admina - zobrazí všetky
  const videaSId = videa.filter(v => v.videoId && v.videoId.trim() !== '');
  const videaBezId = videa.filter(v => !v.videoId || v.videoId.trim() === '');
...
async function otvorModalUpravyMojichPreferencii() {
  ...
  const infoText = document.createElement('p');
  infoText.textContent = 'Tu si môžete nastaviť svoje preferované tímy, sezóny a kategórie. Tieto preferencie sa zobrazia administrátorovi a pomôžu mu pri nastavovaní vašich prístupových práv. Schválenie nastavených preferencií môže trvať aj viac ako 24 hodín.';
  infoText.style.fontSize = '14px';
  infoText.style.color = '#666';
  infoText.style.marginBottom = '20px';
  modalBox.appendChild(infoText);
  ...
  const searchInput = document.createElement('input');
  searchInput.type = 'text';
  searchInput.id = 'teamSearchInputUser';
  searchInput.placeholder = '🔍 Hľadať tímy... (zadajte časť názvu)';
  searchInput.style.width = '100%';
  searchInput.style.padding = '10px 14px';
  searchInput.style.border = '1px solid #ddd';
  ...
  // Počet zobrazených tímov
  const countDisplay = document.createElement('div');
  countDisplay.id = 'teamCountDisplayUser';
  countDisplay.style.fontSize = '12px';
  countDisplay.style.color = '#999';
  countDisplay.style.marginTop = '4px';
  countDisplay.style.textAlign = 'right';
  ...
  container.id = 'teamsContainerUser';
...
function vytvorRegistracnyFormular() {
  ...
  teamLabel.textContent = 'Preferovaný tím, sezóna a kategória (nepovinné)';
  ...
  sezonaSelect.id = 'regSezona';
  sezonaSelect.style.width = '100%';
  sezonaSelect.style.padding = '12px';
  sezonaSelect.style.border = '1px solid #ddd';
  sezonaSelect.style.borderRadius = '4px';
  sezonaSelect.style.fontSize = '14px';
  sezonaSelect.style.boxSizing = 'border-box';
  sezonaSelect.style.marginBottom = '8px';
  ...
  teamGroup.appendChild(sezonaSelect);

  // Riadok pre kategóriu
  const kategoriaSelect = document.createElement('select');
  kategoriaSelect.id = 'regKategoria';
  kategoriaSelect.style.width = '100%';
  kategoriaSelect.style.padding = '12px';
  kategoriaSelect.style.border = '1px solid #ddd';
  kategoriaSelect.style.borderRadius = '4px';
  kategoriaSelect.style.fontSize = '14px';
  kategoriaSelect.style.boxSizing = 'border-box';
  ...
  teamGroup.appendChild(kategoriaSelect);
